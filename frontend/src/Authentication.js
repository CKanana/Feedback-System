...
const Authentication = () => {
  ...
  const handleLogin = async (values, { setSubmitting }) => {
    ...
    try {
      ...
      // 2. Reload user and force token refresh to get latest emailVerified status
      await user.reload();
      await user.getIdToken(true);
      console.log("Firebase Verified Status:", user.emailVerified);
      // Strictly check Firebase for verification status. MongoDB is not used for this check.

      if (!user.emailVerified) {
        // Auto-force verify (Dev Mode) - Automatically verify and retry
        try {
          await axios.post('http://localhost:5000/api/auth/force-verify', { email: user.email });
          ...
          if (err.response && err.response.status === 404) {
            setToastMessage('Dev Error: Backend endpoint not found. Please restart your backend server.');
          } else {
            setToastMessage('Email not verified. Please check your inbox.');
          }
          setShowSuccessToast(true);
      ...
      // 3. Check if user exists in MongoDB, create if not (Self-healing)
      try {
        await axios.get(`http://localhost:5000/api/auth/user-by-email?email=${encodeURIComponent(user.email)}`);
      } catch (err) {
        // Only try to create if the error was specifically "User not found" (404)
        if (err.response && err.response.status === 404) {
          await axios.post('http://localhost:5000/api/auth/signup', {
            name: user.displayName || values.username,
            email: user.email,
          ...
          // If it was a different error (like a server error), rethrow it so login fails properly
          throw err;
        }
      }
      setLoading(false);
    ...
    <div className="auth-wrapper">
      ...
      <div className="auth-right">
        <div className="flip-card">
          <div className={`flip-card-inner ${!isLogin ? 'is-flipped' : ''}`}>
